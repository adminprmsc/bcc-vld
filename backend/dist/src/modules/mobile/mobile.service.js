...
let MobileService = class MobileService {
    ...
    async getDashboard(user) {
        try {
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to load dashboard' });
        }
    }
    async getTasks(user) {
        try {
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to load tasks' });
        }
    }
    async completeTask(id, dto, user) {
        try {
            const ctx = await this.resolveUser(user);
            ...
            if (sample) {
                await this.prisma.water_quality_sample_status_history.create({
                    data: {
                        sample_id: sample.id,
                        code: client_1.water_quality_sample_status_history_code.closed,
                        label: 'Task completed via mobile',
                        note: dto.notes || '',
                        tone: client_1.water_quality_sample_status_history_tone.success,
                ...
                await this.prisma.water_quality_samples.update({
                    where: { id: sample.id },
                    data: {
                        status: dto.status || client_1.water_quality_samples_status.closed,
            ...
            if (plan) {
                await this.prisma.consultant_plan_maintenance_records.create({
                    data: {
                        ...
                        performed_at: now,
                        type: client_1.consultant_plan_maintenance_records_type.inspection,
                        status: client_1.consultant_plan_maintenance_records_status.completed,
            ...
            throw new common_1.NotFoundException({ msg: 'Task not found' });
        }
        catch (err) {
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to complete task' });
        }
    }
    getSyncStatus() {
    ...
    async getMapOverlays() {
        try {
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to load map data' });
        }
    }
        ...
        try {
            ...
            const requisition = await this.prisma.requisitions.create({
                data: {
                    ...
                    priority: dto.priority || client_1.requisitions_priority.Medium,
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to create requisition' });
        }
    }
    async uploadRequisitionAttachments(id, files) {
        try {
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to upload attachments' });
        }
    }
    async createMaintenanceRecord(dto, user) {
        try {
            const ctx = await this.resolveUser(user);
            ...
            if (!plan) {
                throw new common_1.NotFoundException({ msg: 'Asset not found' });
            }
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to record maintenance' });
        }
    }
    async submitWaterSampleCollection(dto, user) {
        try {
            const ctx = await this.resolveUser(user);
            ...
            await this.prisma.water_quality_sample_status_history.create({
                data: {
                    sample_id: sample.id,
                    code: client_1.water_quality_sample_status_history_code.collection_complete,
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to submit collection data' });
        }
    }
    async uploadWaterSampleAttachments(id, files, user) {
        try {
            const ctx = await this.resolveUser(user);
            ...
            for (const f of files || []) {
                const attachment = await this.prisma.water_quality_sample_attachments.create({
                    data: {
                        sample_id: sample.id,
                        attachment_type: client_1.water_quality_sample_attachments_attachment_type.collection,
    ...
    async submitLandUtilizationProgress(dto, user) {
        try {
            const ctx = await this.resolveUser(user);
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to submit progress update' });
        }
    }
    async submitRedbookOperational(dto, user) {
        try {
            const ctx = await this.resolveUser(user);
            const capturedAt = dto.capturedAt ? new Date(dto.capturedAt) : new Date();
            const now = new Date();
            ...
            const attrs = asset.attributes && typeof asset.attributes === 'object' && !Array.isArray(asset.attributes)
                ? { ...asset.attributes }
                : {};
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to submit redbook data' });
        }
    }
    async getNotifications(user) {
        try {
            const ctx = await this.resolveUser(user);
            const notifications = [];
            if ((0, role_synonyms_1.roleMatches)(ctx.role, ['PCRWR Sampler', 'PCRWR Lab'])) {
                    ...
                    where: {
                        ...
                        status: {
                            in: [
                                ...
                                client_1.water_quality_samples_status.awaiting_assignment,
                            ],
                        },
                    },
                });
            ...
            if ((0, role_synonyms_1.roleMatches)(ctx.role, ['DM Tehsil', 'Tehsil Manager'])) {
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to load notifications' });
        }
    }
    async registerPushToken(dto, user) {
        try {
            ...
            throw new common_1.InternalServerErrorException({ msg: 'Failed to register push token' });
        }
    }
    ...
    async getTaskCount(userId, role) {
        let count = 0;
        if ((0, role_synonyms_1.roleMatches)(role, ['PCRWR Sampler', 'PCRWR Lab'])) {
            count += await this.prisma.water_quality_samples.count({
                where: {
                    OR: [{ assigned_sampler: userId }, { lab_analysis_analyst: userId }],
                    status: { notIn: [client_1.water_quality_samples_status.closed, client_1.water_quality_samples_status.cancelled] },
                },
            });
        }
        if ((0, role_synonyms_1.roleMatches)(role, ['Tehsil Manager'])) {
    ...
    async fetchTasks(userId, role) {
        const tasks = [];
        if ((0, role_synonyms_1.roleMatches)(role, ['PCRWR Sampler', 'PCRWR Lab', 'RA Environment'])) {
            ...
            for (const s of samples) {
                tasks.push({
                    ...
                    priority: s.status === client_1.water_quality_samples_status.awaiting_collection ? 'high' : 'medium',
                    dueDate: null,
