...
let SupportService = class SupportService {
        ...
        return {
            ...
            creator: row.users_support_requests_created_byTousers
                ? {
                    ...
                    role: (0, role_util_1.roleEnumToApi)(row.users_support_requests_created_byTousers.role),
                }
                : null,
            assignee: row.users_support_requests_assigned_toTousers
                ? {
                    ...
                    role: (0, role_util_1.roleEnumToApi)(row.users_support_requests_assigned_toTousers.role),
                }
                : null,
            updates: row.support_request_updates.map((u) => ({
                ...
                actor: u.users
                    ? {
                        ...
                        name: u.users.name,
                        email: u.users.email,
                        role: (0, role_util_1.roleEnumToApi)(u.users.role),
                    }
                    : null,
            })),
    ...
    async create(user, body, files = []) {
        ...
        if (!category || !message) {
            throw new common_1.BadRequestException({ msg: 'Category and message are required.' });
        }
        const priority = PRIORITY_VALUES.includes(requestedPriority)
            ? requestedPriority
            : client_1.support_requests_priority.medium;
        ...
        return this.prisma.$transaction(async (tx) => {
            const supportRequest = await tx.support_requests.create({
                data: {
                    ...
                    requester_role: (0, role_util_1.roleEnumToApi)(dbUser.role),
                    category,
                    priority,
                    subject: subject || '',
                    message,
            ...
            const attachmentRecords = files.map((file) => ({
                support_request_id: supportRequest.id,
                stored_name: (0, path_1.join)('support', file.filename).replace(/\\/g, '/'),
    ...
    async patchAdmin(id, user, body) {
        return this.prisma.$transaction(async (tx) => {
            const supportRequest = await tx.support_requests.findUnique({
                where: { id: BigInt(id) },
            });
            if (!supportRequest) {
                throw new common_1.NotFoundException({ msg: 'Support request not found' });
            }
            ...
            if (body.priority && body.priority !== supportRequest.priority) {
                if (!PRIORITY_VALUES.includes(body.priority)) {
                    throw new common_1.BadRequestException({ msg: 'Invalid priority supplied.' });
                }
    ...
    async addComment(id, user, note) {
        if (!note?.trim()) {
            throw new common_1.BadRequestException({ msg: 'Note is required.' });
        }
        ...
        const isAssigned = supportRequest.assigned_to && supportRequest.assigned_to === BigInt(user.userId);
