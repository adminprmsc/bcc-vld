...
import { decimalToNumber } from './requisition.helpers';
...
} satisfies Prisma.requisitionsInclude;
...
export function serializeRequisition(row: RequisitionWithRelations | null) {
  if (!row) {
    return row;
  }
  ...
  const landAcquisitionData = (row.land_acquisition_data || {}) as Record<string, unknown>;
  ...
  return {
    ...
    landBreadth: decimalToNumber(row.land_breadth),
    landDepth: decimalToNumber(row.land_depth),
    calculatedAreaSqFt: decimalToNumber(row.calculated_area_sq_ft),
    calculatedAreaMarlas: decimalToNumber(row.calculated_area_marlas),
    calculatedAreaKanals: decimalToNumber(row.calculated_area_kanals),
    ...
    mapMarker:
      row.map_marker_lat != null && row.map_marker_lng != null
        ? { lat: Number(row.map_marker_lat), lng: Number(row.map_marker_lng) }
        : null,
    mapViewport:
      row.map_viewport_center_lat != null && row.map_viewport_center_lng != null
        ? {
            center: {
    ...
    locationLat: decimalToNumber(row.location_lat),
    locationLng: decimalToNumber(row.location_lng),
    mapMarkerLat: decimalToNumber(row.map_marker_lat),
    mapMarkerLng: decimalToNumber(row.map_marker_lng),
    mapViewportCenterLat: decimalToNumber(row.map_viewport_center_lat),
    mapViewportCenterLng: decimalToNumber(row.map_viewport_center_lng),
...
export function serializeMany(requisitions: RequisitionWithRelations[]) {
  return requisitions.map((row) => serializeRequisition(row));
}

export function summarizeUser(user: unknown): string {
  if (!user) {
    return '—';
  }
